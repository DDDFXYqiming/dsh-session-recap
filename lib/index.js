 ...
 */
function languageDirective(lastUserText) {
    const sample = lastUserText.replace(/\s+/g, ' ').replace(/"""/g, "'''").trim().slice(0, 160);
    if (sample === '') {
        return "\n\n[recap-language] Write the ENTIRE recap in the language of the user-role entries above, regardless of the language of the assistant entries, code, logs, or this directive.";
    }
    return `\n\n[recap-language] The user's own most recent message, verbatim: """${sample}""". Write the ENTIRE recap in exactly the language of that quoted message, regardless of the language of the assistant entries, code, logs, or this directive.`;
}
...
async function generateRecap(ctx, config, session, signal) {
    ...
    const withDirective = framed + languageDirective(lastUserText);
