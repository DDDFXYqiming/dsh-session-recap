...
async function generateRecap(ctx, config, session, signal) {
    if (!config.enabled)
        throw new Error('dsh-session-recap: plugin is disabled');
...
function mountWebRoute(webCtx, store) {
    webCtx.effect(() => {
        const dispose = webCtx.webServer.register({
            ...
            handler: async (req, res) => {
                ...
                const body = { recap: !store.enabled || session === undefined ? null : currentRecap(store, session) };
                if (method === 'HEAD') {
                    res.writeHead(200, {
                        'Content-Type': 'application/json; charset=utf-8',
                        'Cache-Control': 'no-store',
