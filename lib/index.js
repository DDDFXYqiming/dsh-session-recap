 ...
 */
async function streamRecapOnce(ctx, config, route, system, messages, sessionId, signal, maxTokens, salvage) {
...
function mountWebRoute(webCtx, store, setAway, generate, hasManualCommand) {
    webCtx.effect(() => {
...
export function apply(ctx, config) {
    ...
    ctx.inject(['webServer', 'sessions'], (webCtx) => {
        webProfile = true;
        releaseManual();
        mountWebRoute(webCtx, store, setAway, generateManual, () => manualOwned);
    });
