...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RecapBanner(props) {
      ...
      var armed = react.useState(isWindowActive())
      ...
      var activityKey = useSession(function (snapshot) {
        ...
        return String(snapshot.running) + ':' + String(snapshot.pending.length) + ':' + String(order.length) + ':' + (order.length === 0 ? '' : order[order.length - 1])
      })
      ...
      react.useEffect(function () {
        ...
        function syncPresence() {
          var active = isWindowActive()
          setArmed(active)
          reportPresence(sessionKey, active)
        }
        function markAway() {
          setArmed(false)
