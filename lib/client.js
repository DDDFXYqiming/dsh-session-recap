...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function dismissalKey(sessionId, turnSeq) {
      if (sessionId === undefined || sessionId === null || String(sessionId) === '') return null
      return String(sessionId) + ':' + (turnSeq === null || turnSeq === undefined ? 'none' : String(turnSeq))
    }
    ...
    function wasDismissed(sessionId, turnSeq) {
      if (sessionId === undefined || sessionId === null || String(sessionId) === '') return false
      if (typeof window === 'undefined') return false
      var expected = turnSeq === null || turnSeq === undefined ? 'none' : String(turnSeq)
      try {
    ...
    function rememberDismissed(sessionId, turnSeq) {
      if (sessionId === undefined || sessionId === null || String(sessionId) === '') return
      if (typeof window === 'undefined') return
      var value = turnSeq === null || turnSeq === undefined ? 'none' : String(turnSeq)
    ...
    function RecapBanner(props) {
      ...
      var recapState = react.useState(null)
      var recap = recapState[0]
      var setRecap = recapState[1]
      var dismissed = react.useState(null)
      var dismissedKey = dismissed[0]
      var setDismissedKey = dismissed[1]
      var armed = react.useState(typeof document === 'undefined' ? true : !document.hidden)
      ...
      var activityKey = useSession(function (snapshot) {
        ...
        return String(order.length) + ':' + (order.length === 0 ? '' : order[order.length - 1])
      })
        ...
        function load() {
          ...
          fetch(url, { credentials: 'same-origin', cache: 'no-store', signal: controller.signal })
            ...
            .then(function (body) {
              ...
              setRecap(validRecap(body.recap))
            })
            .catch(function () {})
      ...
      var currentKey = dismissalKey(sessionId, turnSeq)
      ...
      react.useEffect(function () {
        ...
        lastActivity.current = activityKey
        if (text !== null && currentKey !== null && shownKey.current === currentKey && !wasDismissed(sessionId, turnSeq)) {
          rememberDismissed(sessionId, turnSeq)
          setDismissedKey(currentKey)
        }
      }, [activityKey, text, currentKey, sessionId, turnSeq])

      react.useEffect(function () {
        if (typeof document === 'undefined') return undefined
        function onVisibility() { setArmed(!document.hidden) }
        document.addEventListener('visibilitychange', onVisibility)
        return function () { document.removeEventListener('visibilitychange', onVisibility) }
      }, [])

      var show = text !== null && currentKey !== null && dismissedKey !== currentKey && !wasDismissed(sessionId, turnSeq) && armed[0]
      react.useEffect(function () {
